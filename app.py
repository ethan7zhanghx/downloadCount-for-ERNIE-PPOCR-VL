...
def run_platforms_parallel(platforms, fetchers_to_use, save_to_database=True):
    ...
    if "AI Studio" in platforms and st.session_state.get('use_model_tree', True):
        ...
        try:
            ...
            mt_elapsed = time.time() - mt_start_time
            model_tree_elapsed += mt_elapsed

            if model_tree_count > 0:
                all_dfs.append(model_tree_df)
                mt_status.success(f"✅ AI Studio Model Tree 完成")
                mt_progress.progress(1.0)
                mt_details.success(f"获取 {model_tree_count} 个衍生模型，用时 {mt_elapsed:.2f} 秒")
            else:
                mt_status.info("ℹ️  未找到新的衍生模型")
                mt_progress.progress(1.0)
                mt_details.info(f"用时 {mt_elapsed:.2f} 秒")

        except Exception as e:
            mt_elapsed = time.time() - mt_start_time
            model_tree_elapsed += mt_elapsed
            mt_status.error(f"❌ Model Tree 失败")
            st.warning(f"⚠️  AI Studio Model Tree 失败（不影响主流程）：{e}，用时 {mt_elapsed:.2f} 秒")

    # ========== 阶段3: ModelScope Model Tree 补充爬取 ==========
    if "ModelScope" in platforms and st.session_state.get('use_model_tree', True):
        # 创建 ModelScope Model Tree 进度显示区域
        st.markdown("### 🌳 ModelScope Model Tree 进度")
        ms_mt_status = st.empty()
        ms_mt_progress = st.progress(0)
        ms_mt_details = st.empty()

        ms_mt_start_time = time.time()

        try:
            from ernie_tracker.fetchers.fetchers_modeltree import update_modelscope_model_tree

            ms_mt_status.info("🔄 正在获取 ModelScope 衍生模型...")

            # 定义进度回调
            def ms_mt_progress_callback(processed, discovered_total=None):
                # 获取官方模型总数（从数据库）
                try:
                    import sqlite3
                    conn = sqlite3.connect(DB_PATH)
                ...
                progress = min(processed / total_official, 1.0) if total_official > 0 else 0
                ms_mt_progress.progress(progress)
                ms_mt_details.info(f"已处理 {processed} / {total_official} 个官方模型")

            # 执行 Model Tree 获取
            model_tree_df, model_tree_count = update_modelscope_model_tree(
                save_to_db=save_to_database,
                auto_discover=True,
                progress_callback=ms_mt_progress_callback
            )

            ms_mt_elapsed = time.time() - ms_mt_start_time
            model_tree_elapsed += ms_mt_elapsed

            if model_tree_count > 0:
                all_dfs.append(model_tree_df)
                ms_mt_status.success(f"✅ ModelScope Model Tree 完成")
                ms_mt_progress.progress(1.0)
                ms_mt_details.success(f"获取 {model_tree_count} 个衍生模型，用时 {ms_mt_elapsed:.2f} 秒")
            else:
                ms_mt_status.info("ℹ️  未找到新的衍生模型")
                ms_mt_progress.progress(1.0)
                ms_mt_details.info(f"用时 {ms_mt_elapsed:.2f} 秒")

        except Exception as e:
            ms_mt_elapsed = time.time() - ms_mt_start_time
            model_tree_elapsed += ms_mt_elapsed
            ms_mt_status.error(f"❌ Model Tree 失败")
            st.warning(f"⚠️  ModelScope Model Tree 失败（不影响主流程）：{e}，用时 {ms_mt_elapsed:.2f} 秒")

    # ========== 最终总结 ==========
    final_elapsed_time = time.time() - total_start_time

    if model_tree_elapsed > 0:
        st.success(f"🎯 全部完成！总用时：{final_elapsed_time:.2f} 秒（阶段1: {total_elapsed_time:.2f}秒，Model Tree: {model_tree_elapsed:.2f}秒）")
    else:
